...
function SportsNewsCard({news}) {
  return (
        <div className=' group cursor-pointer text-black flex flex-col items-center justify-center w-full h-full p-2 gap-y-2 border-1 border-gray-300 rounded-md'>
            ...
            <div className='w-full  h-full flex-1 flex flex-col items-start justify-start overflow-hidden'>
                ...
                <p className='text-lg opacity-60'>{news?.description.length>100?`${news?.description.slice(0,101)}...`:news?.description}</p>
            </div>
        </div>
  )
}
