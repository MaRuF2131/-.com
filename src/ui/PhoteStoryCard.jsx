...
function PhoteStoryCard() {
  ...
  return (
    ...
    {newsData.map((photoStory, index) => (
        <div onClick={()=>click(photoStory?._id)} key={index} className='max-w-full min-w-62 h-80 pt-2 relative'>
          <Image src={photoStory?.images[0]?.imageUrl } alt={photoStory?.title} width={300} height={200} className='object-fill aspect-[5/6.5] rounded-md max-h-full w-full' />
