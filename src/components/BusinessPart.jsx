  ...
  return (
    <div className="w-full max-w-[850px] flex flex-col border-b border-gray-300 pb-4 lg:mx-6 mx-0 ">
      ...
      <div className='grid sm:grid-cols-3 grid-cols-1 w-full'>
            ...
            {/* Left Side */}
           <div className='w-full  pr-4  h-fit  col-span-2 sm:flex hidden flex-wrap items-start justify-center  gap-x-4'>
                {newsData.slice(0,2).map((news,index)=>
                  <div key={index} className={`${index === 1 ? '' : 'border-b border-gray-300'} pb-4 w-full max-h-fit flex items-start justify-center gap-x-4 mb-4`}>
                      <Image src={news?.imageUrl || "/default.webp"} width={300} height={200} className='w-[45%] h-38 object-cover rounded-md' />
            ...
            <div className='w-full flex flex-wrap items-start border-l border-gray-300'>
                {newsData.slice(2,5).map((news,index)=>
                  <div key={index} className={`${index === 2 ? '' : 'border-b border-gray-300'} pb-4 pl-3 w-full max-h-fit flex flex-wrap items-start justify-center gap-x-4 mb-4`}>
                      ...
                      <div className='flex-1 flex gap-2  items-start justify-start mt-2'>
                        ...
                        <Image src={news?.imageUrl || "/default.webp"} width={300} height={200} className='sm:w-[55%] w-[40%] sm:order-2 order-1 sm:h-20 h-25 object-cover rounded-md' />
                      </div>
                  </div>
                )}
           </div>
