...
const fetchData=async (url,keyValuepair,pageParam,limit)=>{
 ...
 function Pagination({url,keyValuepair={},page=1,limit=10}=info) {
    return useInfiniteQuery({
     queryKey: [url,keyValuepair?.country,keyValuepair?.upozila,keyValuepair?.distic,keyValuepair?.division,keyValuepair?.id,keyValuepair?.locationType,keyValuepair?.subcategory,keyValuepair?.category,keyValuepair?.database,keyValuepair?.views],
